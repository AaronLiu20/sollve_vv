//this test uses 6 different sized arrays to perform 3 data transfers wtihin the pragma
...
int main(){
    ...
    for( int x = 0; x < arraySizeLarge; x++){
     ...
     #pragma omp target parallel for num_threads(number_of_threads) \
        ...
        map(from:emptyLarge[0:arraySizeLarge], emptyMedium[0:arraySizeMedium], emptySmall[0:arraySizeSmall])
    for( int x = 0; x < arraySizeLarge; x++){
