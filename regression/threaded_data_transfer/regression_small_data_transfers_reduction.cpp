//this tests uses 6 differently sized arrays and performs 3 data transfers within the pragma while also peforming the +=, -= and *= reductions
...
int main(){
    ...
    for( int x = 0; x < arraySizeLarge; x++){
     ...
     #pragma omp target parallel for num_threads(number_of_threads)\
	 	map(to: dataLarge[0:arraySizeLarge], dataMedium[0:arraySizeMedium], dataSmall[0:arraySizeSmall]) \
		reduction(+:sumReduction) reduction(-:minusReduction) reduction(*:multiplyReduction)\
		map(from: testSmall[0:arraySizeSmall], testMedium[0:arraySizeMedium], testLarge[0:arraySizeLarge]) \
