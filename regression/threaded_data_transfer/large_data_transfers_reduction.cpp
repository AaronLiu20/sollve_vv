//this test uses 16 differently sized arrays and performs 8 data transfers using 10000 threads as well as +=, -=, *=, |=, &=, and ^= reductions
//it peforms three sum reductions since i dont know how to use && and || reductions
...
int main(){
    ...
    #pragma omp target parallel for num_threads(number_of_threads) \
        map(to: data8[0:array_size8], data7[0:array_size7], data6[0:array_size6], data5[0:array_size5], data4[0:array_size4], data3[0:array_size3], data2[0:array_size2], data1[0:array_size1], MultiplcationSize) \
        reduction(+: deviceReduction8) reduction(+:deviceReduction7)  reduction(+:deviceReduction6) reduction(-:deviceReduction5) reduction(*:deviceReduction4) reduction(&: deviceReduction3) reduction(|:deviceReduction2) reduction(^:deviceReduction1) \
        map(from: empty8[0:array_size8], empty7[0:array_size7], empty6[0:array_size6], empty5[0:array_size5], empty4[0:array_size4], empty3[0:array_size3], empty2[0:array_size2], empty1[0:array_size1]) \
        map(tofrom: deviceReduction8, deviceReduction7, deviceReduction6, deviceReduction5, deviceReduction4, deviceReduction3, deviceReduction2, deviceReduction1)
    for( int x = 0; x < array_size8; x++){
    ...
    printf(" (^= Reduction) host = %d device = %d\n (|= Reduction) host = %d device = %d\n (&= Reduction) host = %d device = %d\n (*= Reduction) host = %d device = %d\n (-= Reduction) host = %d device = %d\n (+= Reduction) host = %d device = %d\n (+= Reduction) host = %d device = %d\n (+= Reduction) host = %d device = %d\n", 
        hostReduction1, deviceReduction1, hostReduction2, deviceReduction2, hostReduction3, deviceReduction3, hostReduction4, deviceReduction4, hostReduction5, deviceReduction5, hostReduction6, deviceReduction6, hostReduction7, deviceReduction7, hostReduction8, deviceReduction8);
    return error;
}
